...
parser.add_argument('--scalef', type=float, default=2.5, help='scaling factor in decoder output layer')
...
class VAE(nn.Module):
    ...
    def loss_function(self, x, x_r, mean, log_var, minus_std, plus_std, weight1, weight2, weight3, weight4, weight5, average = False):
        x_np = x.cpu().detach().numpy()  # shape = (batch_size, 11748)
        # batch_size = x.shape(0)
        # count_minus_one = np.count_nonzero(x == -1)
        # count_plus_one = np.count_nonzero(x == 1)
        condlist = [x_np==-1, ((-1<x_np) & (x_np<minus_std)).all(), ((minus_std<=x_np) & (x_np<=plus_std)).all(),  (plus_std<x_np) & ((x_np<1)).all(), x_np==1]
        choicelist = [weight1, weight2, weight3, weight4, weight5]
        print('choicelist: ')
        print(choicelist)
        loss_scaler = np.select(condlist, choicelist)
        loss_scaler = torch.from_numpy(loss_scaler).to(DEVICE)
        print('loss_scaler: ')
        print(loss_scaler)
...
def scatter_comparison(x_samp, x_r_samp):
    ...
    fig.savefig('/hpc/compgen/users/cchang/Projects/gene_exp_VAE/data/tanh_scalef/images/512/val_scatter_com_batch-un-corrected_' +  str(args.cvbs) + '_' + str(args.conf) + 'batch0_sample0' + '.png')
...
def get_group_size(x):
    ...
    count_group1 = (x<minus_std).sum() - count_minus_one
    count_group2 = np.count_nonzero(np.logical_and(x>=minus_std, x<plus_std))
    count_group3 = len(x) - (count_minus_one + count_group1 + count_group2 + count_plus_one)

    print(count_minus_one + count_group1 + count_group2 + count_group3 + count_plus_one)

    return minus_std, plus_std, count_minus_one, count_group1, count_group2, count_group3, count_plus_one

def get_weight(group_sizes, no_of_classes=5, beta = 0.999):  # https://towardsdatascience.com/handling-class-imbalanced-data-using-a-loss-specifically-made-for-it-6e58fd65ffab
    effective_num = 1.0 - np.power(beta, group_sizes)
...
def main():
    ...
    with open('/hpc/compgen/users/cchang/Projects/gene_exp_VAE/data/tanh_scalef/log_files/train_SyNet_VAE_batch-un-corrected_' + str(args.cvbs) + '_' + str(args.conf) + '.log', 'w') as log:
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, x in enumerate(train_dataloader):  # How does the train_dataloader look like? We don't have labels. 
            ...
            x = x.view(len(x), train_inp_size)  # x shape is (batch_size, 11748) now. Returns a new tensor with the same data but of a different shape as given.
            minus_std, plus_std, count_minus_one, count_group1, count_group2, count_group3, count_plus_one = get_group_size(x=x.cpu().detach().numpy())
            x = x.to(torch.float32)
            x = x.to(DEVICE)

            z, mu, log_var, x_r= model(x)
            
            count_list = [count_minus_one, count_group1, count_group2, count_group3, count_plus_one]
            weights = get_weight(count_list)
            train_reconstruction_loss, train_loss = model.loss_function(
                x_r=x_r, x=x, mean=mu, log_var=log_var, minus_std=minus_std, plus_std=plus_std, 
                weight1=weights[0], weight2=weights[1], weight3=weights[2], weight4=weights[3], weight5=weights[4]
                )
        ...
        with torch.no_grad():
            ...
            for batch_idx, x in enumerate(train_dataloader):  
                ...
                count_list = [count_minus_one, count_group1, count_group2, count_group3, count_plus_one]
                weights = get_weight(count_list)
                train_reconstruction_loss_corrected, train_loss_corrected = model.loss_function(
                x_r=x_r, x=x, mean=mu, log_var=log_var, minus_std=minus_std, plus_std=plus_std, 
                weight1=weights[0], weight2=weights[1], weight3=weights[2], weight4=weights[3], weight5=weights[4]
                )
            ...
            for batch_idx, x in enumerate(val_dataloader):
                ...
                val_reconstruction_loss, val_loss = model.loss_function(
                x_r=x_r, x=x, mean=mu, log_var=log_var, minus_std=minus_std, plus_std=plus_std, 
                weight1=weights[0], weight2=weights[1], weight3=weights[2], weight4=weights[3], weight5=weights[4]
                )
        ...
        if average_val_loss < 1500: 
            torch.save({
                'epoch': epoch+1,
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict()}, 
                '/hpc/compgen/users/cchang/Projects/gene_exp_VAE/data/tanh_scalef/trained_model_state_dict/state_dict_' + str(args.conf) + '_' + str(args.cvbs) + '_epoch' + str(epoch + 1) + '.pt'
                )
        with open('/hpc/compgen/users/cchang/Projects/gene_exp_VAE/data/tanh_scalef/log_files/train_SyNet_VAE_batch-un-corrected_' + str(args.cvbs) + '_' + str(args.conf) + '.log', 'a') as log:
            csv_writer = csv.writer(log)
    ...
    print('min val_loss: ' + str("%.3f" % min(val_loss_history)) + ', Epoch: ' + str(val_loss_history.index(min(val_loss_history))+1) + ', min val_rec_loss: '+ str("%.3f" % min(val_reconstruction_loss_history)) + ', Epoch: ' + str(val_reconstruction_loss_history.index(min(val_reconstruction_loss_history))+1) + ', max val_pearson: ' + str("%.3f" % max(pearson_history_val)) + ', Epoch: ' + str(pearson_history_val.index(max(pearson_history_val))+1) + ', Total trainable parameters: ' + str(num_trainable_param))
    ...
    ax2.legend()
    ax3.legend()
    plt.savefig('/hpc/compgen/users/cchang/Projects/gene_exp_VAE/data/tanh_scalef/images/512/loss_history_batch-un-corrected_' +  str(args.cvbs) + '_' + str(args.conf) +'.png')
