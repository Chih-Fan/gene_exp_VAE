...
def hexbin(x_samp, x_r_samp, pear_corr):
    ...
    # fig.savefig('/hpc/compgen/users/cchang/Projects/gene_exp_VAE/data/new0501/train_all/images/average_hexbin_' + str(args_.bc) + '_' + str(args_.cvbs) + '.png')
...
def umap_latent(mu, mu_std, y, s, save):
    ...
    # plt.savefig('/hpc/compgen/users/cchang/Projects/gene_exp_VAE/data/new0501/train_all/images/UMAP_latent_surv_' + str(save) + '_'  + str(args_.bc) + '_' +  str(args_.cvbs) + '_' + str(args_.conf) + '.png', bbox_inches='tight', dpi=500)
    ...
    # plt.savefig('/hpc/compgen/users/cchang/Projects/gene_exp_VAE/data/new0501/train_all/images/UMAP_latent_study_' + str(save) + '_'  + str(args_.bc) + '_' +  str(args_.cvbs) + '_' + str(args_.conf) + '.png', bbox_inches='tight', dpi=500)
...
def run_inference(gene_exp_file, dict_file, surv_labels_df, split_file=None):
    ...
    torch.save(mu_std_cat, '../data/new0501/embedding_tensors/embedding_' + args_.cvbs + '_' + str(args_.conf) +'.pt')
    torch.save(y_cat, '../data/new0501/embedding_tensors/surv_labels_' + args_.cvbs + '_' + str(args_.conf) +'.pt')
    torch.save(s_cat, '../data/new0501/embedding_tensors/study_labels_' + args_.cvbs + '_' + str(args_.conf) +'.pt')
...
def log_reg_all(model, x, y, x_test, y_test):
    ...
    # ax.axis("square")
    ax.legend()
    ax.set_title('VAE + log_reg final test AUPRC')
    fig.savefig('../data/new0501/train_all/images/prc_' + str(args_.conf) + '_' + str(args_.cvbs) + '.png')
    ...
    # fig2.savefig('../data/new0501/train_all/images/confusion_mat_' + str(args_.conf) + '_' + str(args_.cvbs) + '.png')
...
def main():
    ...
    surv_labels_file = pd.read_csv('../data/Survival_labels_filtered.csv', index_col=[0], header=[0])
    test_labels_df = surv_labels_file[surv_labels_file.index.str.contains('Miller|Minn')]
    ...
    x = torch.load('../data/new0501/embedding_tensors/full_mu_std_embedding_conf41.pt', map_location=DEVICE)
    y = torch.load('../data/new0501/embedding_tensors/full_surv_labels_conf41.pt', map_location=DEVICE)
    log_reg_all(model=LogisticRegression(penalty='l2',solver='lbfgs', max_iter=1500), 
                x=x.to(DEVICE), y=y.to(DEVICE), x_test=mu_std_test, y_test=y_test)
