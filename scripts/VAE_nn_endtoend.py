    ...
    for fold, (train_index, val_index) in enumerate(logo.split(x_array, y_array, groups=groups)):
        ...
        for epoch in range(args.epochs):
            ...
            for batch_idx, (x, y) in enumerate(train_loader):
                ...
                acc = accuracy_score(y.detach().cpu().numpy(), y_pred_class.detach().cpu().numpy())
                f1 = f1_score(y.detach().cpu().numpy(), y_pred_class.detach().cpu().numpy())
                total_train_acc += acc
                total_train_f1 += f1
            ...
            average_train_eval_acc_ep = total_train_acc / (len(train_loader)+1) *100
            average_corrected_train_eval_acc_ep = total_corrected_train_acc / (len(train_loader)+1) *100
            average_val_eval_acc_ep = total_val_acc / (len(val_loader)+1) *100
            ...
            average_train_f1_ep = total_train_f1 / (len(train_loader)+1)
            average_corrected_train_f1_ep = total_corrected_train_f1 / (len(train_loader)+1)
            average_val_f1_ep = total_val_f1 / (len(val_loader)+1)
